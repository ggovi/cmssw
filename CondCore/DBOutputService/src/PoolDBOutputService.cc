...
using throwException = cond::persistency::throwException;
  ...
  //m_logheaders()
{
  ...
  m_session = connection.createSession( iConfig.getParameter<std::string>("connect") ); 
...
cond::service::PoolDBOutputService::initDB( bool forReading )
{
  ...
  try{ 
    ...
    throwException( std::string(er.what()) + " from PoolDBOutputService::initDB" );
  }
  scope.close();
  m_dbstarted=true;
}
...
cond::service::PoolDBOutputService::createNewIOV( const Hash& firstPayloadId,
  ...
  if(!myrecord.m_isNewTag) {
    throwException( myrecord.m_tag + " is not a new tag", "PoolDBOutputService::createNewIOV");
  }
  std::string iovToken;
  //if(withlogging){
  //  if( m_logConnectionString.empty() ) {
  ...
  try{
    ...
    //}
    throwException(std::string(er.what()) + " from PoolDBOutputService::createNewIOV ",
		   "PoolDBOutputService::createNewIOV");
  }
  scope.close();
}
...
cond::service::PoolDBOutputService::appendSinceTime( const Hash& payloadId,
  ...
  Record& myrecord=this->lookUpRecord(recordName);
  if( myrecord.m_isNewTag ) {
    throwException(std::string("Cannot append to non-existing tag ") + myrecord.m_tag,
  ...
  try{
    ...
    //}
    throwException(std::string(er.what()),
		   "PoolDBOutputService::appendSinceTime");
  }
  scope.close();
}
...
cond::service::PoolDBOutputService::lookUpRecord(const std::string& recordName){
  ...
  if(it==m_callbacks.end()) {
    throwException("The record \""+recordName +"\" has not been registered.","PoolDBOutputService::lookUpRecord");
  }
...
cond::service::PoolDBOutputService::closeIOV(Time_t lastTill, const std::string& recordName, 
  ...
  if( myrecord.m_isNewTag ) {
    throw cond::Exception(std::string("Cannot close non-existing tag ") + myrecord.m_tag,
