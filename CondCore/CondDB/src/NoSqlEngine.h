...
namespace cond {
  ...
  namespace persistency {
    ...
    class NoSqlEngine {
    private:
      static SessionImpl::BType backendFromConnStr( const std::string& connStr ) {
        ...
        if (found!=std::string::npos) return SessionImpl::REST;
        return SessionImpl::UNKNOWN_DB; 
      } 

      /* This template function creates the required SessionImpl instance for the ConnectionPool. */
      template<class T, class T2>
      static std::shared_ptr<SessionImpl> create( const std::string& connStr, SessionImpl::BType bt ) {
    ...
    public:
      static SessionImpl::BType backend( const std::string& connStr ) {
      ...
      static std::shared_ptr<SessionImpl> sessionImpl( const std::string& connStr ) {
         SessionImpl::BType bt = backendFromConnStr( connStr );
         switch( bt ) {
           //case MONGO_DB:
#warning "FIXME: Hardcoded connection strings... (R.S.)"
           ...
           //  break;
	 case SessionImpl::REST: {
	     // cut off rest:// 
             
	     std::string serverName = connStr.substr(7);
             serverName = "http://"+serverName;
	     std::cout <<"SERVERNAME = "<<serverName<<std::endl;
             return create<RestSession, RestTransaction>( serverName, bt);
	 } break;
         case SessionImpl::UNKNOWN_DB:
             throwException( "NoSqlEngine cannot create SessionImpl for connection string: " + connStr, "NoSqlEngine::create" );
             break;
	 case SessionImpl::COND_DB: /* Never reached in this context -> backendFromConnStr(...) */
	   break;
